...
public class Exercise 
{
	...
	{
		Timer timer = new Timer(1, new MyTimerEx());
...
class MyTimerEx implements TimerListener 
{
	boolean setPrevious = false, error50 = false, firstSample = false, movingLeft = false;
	public ArrayList<Integer> sensors = new ArrayList<>();
	double count = 0;
	boolean leftSeenWhite = false;
	boolean rightSeenWhite = false;
	float max, maxPercent, maxLow, maxHigh, min, minPercent, minLow, minHigh;
	float outerLeftMax = 0, outerLeftMin = 0, 
			innerLeftMax = 0, innerLeftMin = 0, 
			innerRightMax = 0, innerRightMin = 0, 
			outerRightMax = 0, outerRightMin = 0;
	private int rightModVal = 8;
	private int leftModVal = 8;
	private int error = 0, previousError = 0;
	private static int outerLeftSensorPrev = 0, innerLeftSensorPrev = 0, innerRightSensorPrev = 0, outerRightSensorPrev = 0;
	int correction = 0;
	double coefficientP = -.35;
	double coefficientD = -.15;
	int speed = 87;
...
//		int innerLeftSensorSample = normalize(innerLeftColorSample[0], "IL");
	...
	public int[] determineSensorPair()
	{
	...
	public int getMin(ArrayList<Integer> list, boolean initPass)
	{
		int index = -1;
		if(initPass)
		{
	...
	public float calculateMaxMin(float[] samples, boolean getMax)
	{
	...
	private int normalize(float reading, String sensor)
	{
		int result;
